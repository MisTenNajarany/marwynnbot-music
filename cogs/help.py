...
class Help(commands.Cog):
    ...
    async def help(self, ctx, *, name: str = None):
        ...
        timestamp = f"Executed by {ctx.author.display_name} " + "at: {:%m/%d/%Y %H:%M:%S}".format(datetime.now())
        embed = discord.Embed(title="MarwynnBot Music Help Menu",
        ...
        embed.set_author(name="MarwynnBot Music", icon_url=ctx.me.avatar_url)
        embed.set_footer(text=timestamp, icon_url=ctx.author.avatar_url)
    ...
    @commands.command(desc="Fetches the invite link to MarwynnBot Music's Support Server",
                      usage="support")
    async def support(self, ctx):
        embed = discord.Embed(title="Support Server",
                              ...
                              "Thank you for using MarwynnBot Music!",
                              color=discord.Color.blue())
        embed.set_thumbnail(url=self.bot.user.avatar_url)
        return await ctx.channel.send(content="https://discord.gg/78XXt3Q", embed=embed)

    @commands.command(desc="Gives the user a link to MarwynnBot Music's privacy policy",
                      usage="privacy")
